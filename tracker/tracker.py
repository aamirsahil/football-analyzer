...
class Tracker:
    ...
    def detect_frames(self, frames):
        ...
        for i in range(0, len(frames), batch_size):
            detections_batch = self.model.predict(frames[i:min(i+batch_size, len(frames))], conf=0.1)
            detections += detections_batch

        return detections

    def get_object_tracks(self, frames, read_from_stub=False, stub_path=""):
        ...
        for frame_num, detection in enumerate(detections):
            ...
            # convert detection into supervision format
    ...
    def draw_triangle(self, frame, bbox, color):
        ...
        triangle_points = np.array(
            [[x,y], [x-buffer, y-2*buffer], [x+buffer, y-2*buffer]]
            )
    ...
    def draw_team_ball_control(self, frame, frame_num, team_ball_control):
        ...
        # get number of time each team had ball
        ...
        cv2.putText(frame, f"Team 1 Ball control : {team_1*100 : .2f} %", (1350, 900), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 0), 3)
        cv2.putText(frame, f"Team 2 Ball control : {team_2*100 : .2f} %", (1350, 950), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 0), 3)
